...
class HungerGames extends PluginBase implements Listener {
	...
	{
            $this->getLogger()->info(TextFormat::GREEN . "HungerGames by PeceteMAN");
						...
						if($args[0]=="make")
						{
							if(!empty($args[1]))
							{
								if(file_exists($this->getServer()->getDataPath() . "/worlds/" . $args[1]))
								{
									...
									$this->mode = 1;
									$player->sendMessage($this->prefix . "Arena Kayıt Bölgesi Seçildi!");
									$player->setGamemode(1);
									$player->teleport($this->getServer()->getLevelByName($args[1])->getSafeSpawn(),0,0);
								}
								else
								{
									$player->sendMessage($this->prefix . "Böyle bir isimde dünya yok.");
								}
							}
							else
							{
								$player->sendMessage($this->prefix . "Parametre Uyuşmuyor.");
							}
						}
						else
						{
							$player->sendMessage($this->prefix . "Böyle bir komut bulunmakta.");
						}
					}
					else
					{
						$player->sendMessage($this->prefix . "Parametreler Uyuşmuyor.");
					}
				}
			return true;
			case "setrank":
				...
				if(!empty($args[0]))
				{
					...
					$config->save();
					$player->sendMessage($args[1] . " Elde edilen aralık: " . $rank);
					}
					else
					{
						$player->sendMessage("Eksik Parametreler.");
					}
				}
				else
				{
	...
	public function onInteract(PlayerInteractEvent $event)
	{
		...
		if($tile instanceof Sign) 
		{
			if($this->mode==26)
			{
				$tile->setText(TextFormat::AQUA . "[GIRIS]",TextFormat::YELLOW  . "0 / 24",$this->currentLevel,$this->prefix);
				$this->refreshArenas();
				$this->currentLevel = "";
				$this->mode = 0;
				$player->sendMessage($this->prefix . "Arena'ya giriş yapıldı!");
			}
			else
			{
				$text = $tile->getText();
				...
				{
					if($text[0]==TextFormat::AQUA . "[GIRIS]")
					{
						$config = new Config($this->getDataFolder() . "/config.yml", Config::YAML);
					...
					{
						$player->sendMessage($this->prefix . "Bu Oyuna Giremeyiz. Dolu veya bitmek üzere.");
					}
				}
			}
		}
		else if($this->mode>=1&&$this->mode<=24)
		{
			...
			$player->sendMessage($this->prefix . "Spawn " . $this->mode . " Yeri seçildi.");
			$this->mode++;
			if($this->mode==25)
			{
				$player->sendMessage($this->prefix . "Deathmatch yeri seç! ");
			}
			$config->save();
		}
		else if($this->mode==25)
		{
			...
			$player->sendMessage($this->prefix . "Lobiye Isinlaniyorsun. Tabelaya tikla ve islem bitsin.");
			$spawn = $this->getServer()->getDefaultLevel()->getSafeSpawn();
			$this->getServer()->getDefaultLevel()->loadChunk($spawn->getFloorX(), $spawn->getFloorZ());
			$player->teleport($spawn,0,0);
			$config->save();
		...
		foreach($tiles as $t) {
			if($t instanceof Sign) {	
				...
				if($text[3]==$this->prefix)
				{
					...
					$ingame = TextFormat::AQUA . "[GIRIS]";
					$config = new Config($this->plugin->getDataFolder() . "/config.yml", Config::YAML);
					if($config->get($text[2] . "PlayTime")!=780)
					{
						$ingame = TextFormat::DARK_PURPLE . "[Bitmek Üzere]";
					}
					else if($aop>=24)
					{
						$ingame = TextFormat::GOLD . "[DOLU]";
					}
					...
					else
					{
						if(count($playersArena)>=2)
						{
							if($timeToStart>0)
							{
								...
								{
									$pl->sendTip(TextFormat::GOLD . $timeToStart . " segundos");
								}
								if($timeToStart<=0)
								{
							...
							else
							{
								...
								if($aop==1)
								{
									...
									{
										$pl->sendMessage($this->prefix . TextFormat::GREEN . "Oyun Kazanıldı");
								...
								if(is_int($minutes) && $minutes>0)
								{
									foreach($playersArena as $pl)
									{
										$pl->sendMessage($this->prefix . $minutes . "Dakika Kaldı Oyunun Bitmesine" );
									}
								}
								else if($time2 == 300)
								{
									foreach($playersArena as $pl)
									{
										$pl->sendMessage($this->prefix . "SANDIKLAR YENILENDI! ORTAYA KOS! ");
									}
									$this->refillChests($levelArena);
								}
								else if($time2 == 30 || $time2 == 15 || $time2 == 10 || $time2 ==5 || $time2 ==4 || $time2 ==3 || $time2 ==2 || $time2 ==1)
								{
									foreach($playersArena as $pl)
									{
										$pl->sendMessage($this->prefix . $time2 . " Saniye Kaldı Oyunun Bitmesine");
									}
								}
								if($time2 <= 0)
								{
								...
								else
								{
									...
									else if($time == 30 || $time == 15 || $time == 10 || $time ==5 || $time ==4 || $time ==3 || $time ==2 || $time ==1)
									{
										foreach($playersArena as $pl)
										{
											$pl->sendMessage($this->prefix . $time . " Saniye Kaldı Oyunun Bitmesine");
										}
									}
									if($time <= 0)
									{
										...
										{
											$pl->teleport($spawn,0,0);
											$pl->sendMessage($this->prefix . "Kimse Kazanamadı!");
											$pl->getInventory()->clearAll();
										}
										$time = 780;
									}
						...
						else
						{
							if($timeToStart<=0)
							{
								foreach($playersArena as $pl)
								{
									$pl->sendMessage($this->prefix . TextFormat::GREEN . "OYUN KAZANILDI!");
									$pl->getInventory()->clearAll();
									$spawn = $this->plugin->getServer()->getDefaultLevel()->getSafeSpawn();
									$this->plugin->getServer()->getDefaultLevel()->loadChunk($spawn->getX(), $spawn->getZ());
									$pl->teleport($spawn);
							...
							else
							{
								foreach($playersArena as $pl)
								{
									$pl->sendTip(TextFormat::RED . "Oyuncular Bekleniyor");
								}
								$config->set($arena . "PlayTime", 780);
								$config->set($arena . "StartTime", 60);
							}
